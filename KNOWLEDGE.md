...
**Next-session entry point:** Resume at one of (a) execute `scripts/load_test_m2b.py --duration 1800 --concurrency 4 --out logs/m2b_$(date +%F_%H%M).json` on EVO-X2 to retire M2b, (b) live smoke-test the new /chat and /v1/chat/completions on EVO-X2 against the running llama-server (verify thinking-mode resolution, REASONING_INBAND opt-in path, /health new field shape), or (c) move on to **M6** (parallelize RAG retrieval + worker dispatch with asyncio.gather) or the Hermes adoption H1 pre-flight block. M1 source: **bartowski/Qwen_Qwen3-30B-A3B-Instruct-2507-GGUF Q5_K_M** at `~/Git/Project_Persona/models/Qwen_Qwen3-30B-A3B-Instruct-2507-Q5_K_M.gguf` (21.74 GB, moved from `~/Live/AIStack/...` on 2026-05-16). M2a verified: Vulkan via RADV on AMD Radeon Graphics RADV GFX1151, llama.cpp b8157, uma=1, fp16=1, bf16=0, KHR_coopmat present.
...
- Sustained-load stability on Vulkan/RADV under multi-slot load is unverified — M2a confirmed build + boot, M2b script landed 2026-05-17 but the actual 30+ minute run hasn't executed yet. Run before relying on the unified server for long agentic workloads.
...
| M2b | Sustained-load stability test on Strix Halo — verify no thermal throttling / driver hangs / KV cache corruption under 30+ minute parallel-slot dispatch. **Script landed 2026-05-17** at `scripts/load_test_m2b.py` (asyncio + httpx, 4 concurrent workers, per-minute throughput buckets, /health poll, JSON report). Run pending. | High | Migration |
