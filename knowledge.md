...
scripts/                      ops scripts (status, doctor, start/stop, tests, setup)
...
Runtime tunables currently live in `run/llama-servers.env` (llama-server flags)
and `scripts/start_api.sh` (API, reasoning, thinking-mode vars). Consolidation
into a single `run/config.env` is planned (see `todo.md`). Target unified-topology
values:
...
Planned shape changes, not immediate todos (those live in `todo.md`).
