...
- Phase 4 Godot client + Phase 5 voice: DONE + PROVEN 2026-06-29 on the Windows client (clients/).
  Avatar visually confirmed on-screen (STATE-driven face: emotion->color, gesture, mouth); voice
  proven end-to-end incl. LIVE hardware (sounddevice installed; full mic -> STT -> /chat -> TTS ->
  speaker round-trip; VAD auto-stop turn-taking). To actually hold a spoken conversation:
  clients\voice\run_voice.ps1 converse (speak; a pause ends each turn; Ctrl+C to stop).
- NOT yet committed (deferred per Brandon: "worry about git" after the voice work). When ready:
  commit the clients/ tree + tracker updates locally; push stays Brandon's call (WORKFLOW.md).
