refer to this file for conventuins D:\Projects\WORKFLOW.md

# Project_Persona — Knowledge & Task Tracker
**Last Updated:** 2026-05-23 0918 PDT
...
**Reference handoff:** `archive/handoffs/HANDOFF_2026-05-10_1738_agent-swarm-hermes-adoption.md`
...
**Status:** Approved, migration substantially complete. M1 resolved 2026-05-14. M2a resolved 2026-05-14 (build verified). **M3 + M4 resolved 2026-05-16** via qwen-test first-boot: env + launcher rewritten for unified Qwen3-30B-A3B, Vulkan device pinning landed, dry-run + actual launch + end-to-end smoke test all PASS at ~63 tok/s gen / ~67 tok/s prompt-eval on EVO-X2 iGPU. **M2b RESOLVED 2026-05-23** — 30-min sustained-load run on EVO-X2 returned 2066/2066 OK, gen_tps_mean=28.26 per slot, per-minute throughput flat across the window, stability ghost did not recur. See archive/handoffs/HANDOFF_2026-05-23_0218. **M5 resolved 2026-05-17** via server.py rewrite: SCIENTIST_URL/PORT retired, ASYNC_SCIENTIST_* → ASYNC_REASONING_* + SCIENTIST_INBAND_* → REASONING_INBAND_* (with back-compat env fallback), prompt-level `/think` / `/no_think` directives added via `thinking_prefix()` helper, PERSONA_CONCURRENCY default raised 2→4 and gate added to /v1/chat/completions, /health exposes new toggles. Live smoke test on EVO-X2 still pending. **M7 partially landed** (env wiring of `cache_prompt: true` deferred to T2.1 sampling overhaul). **M8 deferred** to T2.4 — `<think>` stripping at output boundary.
**Reference handoffs:** `archive/handoffs/HANDOFF_2026-05-09_0250_single-model-migration.md`, `archive/handoffs/HANDOFF_2026-05-16_2337_qwen-test-first-boot.md`, `archive/handoffs/HANDOFF_2026-05-17_0730_qwen-test-canonicalize.md`, `archive/handoffs/HANDOFF_2026-05-17_1030_m5-server-py-migration.md`
**Next-session entry point:** M2b retired 2026-05-23. Resume at one of (a) **M6** (parallelize RAG retrieval + worker dispatch with `asyncio.gather`, replacing the serial in-band reasoning call), (b) the **Hermes adoption H1 pre-flight block** (H1.1 read docs end-to-end → H1.5 egress integration test), (c) live smoke-test the new `/chat` + `/v1/chat/completions` thinking-mode/REASONING_INBAND paths beyond what M2b's single-shot prompts exercised, or (d) housekeeping: `scripts/load_test_m2b.py` `DEFAULT_ENDPOINT` drift (8080→8090), `start_api.sh` cosmetic banner drift (SCIENTIST_* still referenced), and the `prompt_tokens=0` suspect in API responses (token-counter bug?). M1 source: **bartowski/Qwen_Qwen3-30B-A3B-Instruct-2507-GGUF Q5_K_M** at `~/Git/Project_Persona/models/Qwen_Qwen3-30B-A3B-Instruct-2507-Q5_K_M.gguf` (21.74 GB, moved from `~/Live/AIStack/...` on 2026-05-16). M2a verified: Vulkan via RADV on AMD Radeon Graphics RADV GFX1151, llama.cpp b8157, uma=1, fp16=1, bf16=0, KHR_coopmat present.
...
| Reasoning quality guard | ❌ NOT present — decision recorded 2026-05-17 | `looks_degenerate()` + two-stage self-repair loop are NOT in live `services/api/server.py`. **Decision 2026-05-17:** DEFER reinstate to tier-T2 work (output-boundary filter cluster — natural neighbor of T2.4 `<think>`-stripping). Tracked as TODO #37 with explicit gate: land or formally drop before T2.4 closes. |
| Reasoning fallback to persona | ✅ Present | REASONING_FALLBACK_TO_PERSONA toggle — off by default |
| Async reasoning routing | ⚠️ Off by default | `ASYNC_REASONING_ENABLED=0` (back-compat reads `ASYNC_SCIENTIST_ENABLED`) — topics: science,biology,coding,math |
...
- ~~`build_persona_prompt()` is a placeholder — `SOUL.md` (was persona.md) and `.hermes.md` (was system_rules.md) not loaded by API yet~~ — **RESOLVED 2026-05-17 (M5 session):** loader switched to 2-file Hermes naming, prompt prefix uses Soul/Hermes sections, Style guide section dropped.
...
- Profile wrappers (`SOUL.md`, `.hermes.md`) loaded into every prompt — `_read_text` allows up to 12000 chars per file; worst-case ~6K tokens of static context (down from ~9K with the prior 3-file structure since `style.md` is being removed). Mitigation: trim aggressively and/or use `cache_prompt: true` on llama-server requests. Less pressing post-migration since Qwen3 has 32K native context (extendable to ~262K via YaRN per the published model config — not 128K as previously noted here).
- ~~API-side `PERSONA_CONCURRENCY=2` semaphore gates parallelism upstream of llama.cpp's slot scheduler — now under-sized vs the 4 parallel slots the unified server exposes. M5 raises or removes the cap.~~ — **RESOLVED 2026-05-17 (M5):** default raised to 4 (env-driven); gate now applied to both `/chat` and `/v1/chat/completions` for consistency. Tunable via `PERSONA_CONCURRENCY` in `run/llama-servers.env`.
- **Chat template auto-detected as Hermes 2 Pro** on the unified server (2026-05-16 launch log). Hermes 2 Pro is a ChatML superset with native tool-calling — already the expected format for Qwen3 and the format Hermes Agent (DECISION 2026-05-11) speaks. **One less item on the Phase 8 H1 pre-flight checklist** — no custom chat template work needed.
- ~~Sustained-load stability on Vulkan/RADV under multi-slot load is unverified~~ — **RESOLVED 2026-05-23.** M2b 30-min run on EVO-X2: 2066/2066 OK, zero errors, gen_tps_mean=28.26 per slot, per-minute flat. Peak-load thermal data not captured (only post-test idle sample taken at +1h23m, uninformative); follow-up: parallel `sensors` log during next baseline pass.
...
| Tested (legacy dual-server) | 96GB | AMD Ryzen AI APU (Vulkan) | 35+45 layer offload | GMKtec EVO-X2, retired 2026-05-16 |
| **Tested (unified Qwen3)** | **96GB unified** | **AMD Radeon Graphics RADV GFX1151 (Vulkan, uma=1, fp16=1, bf16=0)** | **Qwen3-30B-A3B Q5_K_M, 49/49 layers, 4 parallel slots, 32K ctx, q8_0 KV cache, Flash Attention** | **GMKtec EVO-X2 / Strix Halo iGPU. Smoke-tested ~63 tok/s gen, ~67 tok/s prompt eval (single-slot single-prompt). M2b sustained-load (2026-05-23): gen_tps_mean=28.26 per slot × 4 slots ≈ 113 tok/s aggregate, p50=4.358s, zero errors over 30 min, throughput flat.** |
...
- [x] ~~Wire `SOUL.md` + `.hermes.md` into build_persona_prompt() — currently placeholder~~ — **Done 2026-05-17 (M5 session).** Loader switched to 2-file Hermes naming; style.md retired.
...
- ~~`looks_degenerate()` — quality heuristic~~ — **NEVER LANDED**. Spec'd here historically; verified absent from `services/api/server.py` (2026-04-27 grep, re-verified 2026-05-17). Decision 2026-05-17: deferred to T2-tier output-boundary work (TODO #37). Either reinstate alongside T2.4 `<think>`-stripping or formally drop. Do not treat as live behavior.
- ~~Two-stage self-repair loop~~ — same status as above. Coupled to `looks_degenerate()` and shares its decision gate.
- `reasoning_lock` — one reasoning job at a time
- `persona_sem` — persona concurrency cap (PERSONA_CONCURRENCY=2) — keeps persona responsive while experts run. Under-sized vs unified server's 4 slots; M5 raises or removes.
...
**Phase 8 deliverables — see `archive/handoffs/HANDOFF_2026-05-10_1738_agent-swarm-hermes-adoption.md` H1–H6**
...
> Reference handoff: `archive/handoffs/HANDOFF_2026-05-15_0127_compat-reeval-tiered.md`
>
...
| M2b | ~~Sustained-load stability test on Strix Halo — verify no thermal throttling / driver hangs / KV cache corruption under 30+ minute parallel-slot dispatch~~ — **RESOLVED 2026-05-23.** 30-min run @ concurrency=4 against unified Qwen3-30B-A3B Q5_K_M on EVO-X2: 2066/2066 OK, 60/60 health polls OK, lat_p50=4.358s / p95=4.553s / max=4.763s, gen_tps_mean=28.26 per slot (~113 tok/s aggregate). Per-minute throughput essentially flat (range 27.78–28.79 from min00→min29, no degradation curve). Pre-test cold idle: CPU Tctl 34.2°C, iGPU edge 33.0°C, PPT 6.05 W. **Peak-load thermal not captured** — only sample taken post-test was at +1h23m after wrap, by which time the iGPU had returned to idle, so it carries no information about behavior under load. Next baseline pass should parallel-log `sensors` during the run. **Stability ghost** from §Open issues #1 (HANDOFF_2026-05-19/20) did NOT recur — llama-server held through the entire window plus 30+ min beyond. Report: `logs/m2b_2026-05-23_0723.json`. See archive/handoffs/HANDOFF_2026-05-23_0218_docs-drift-m2b-baseline.md. | ✅ Done | Migration |
| M3 | ~~Update `run/config.env` — single-model section, `PARALLEL_SLOTS`, drop reasoning/coder sections~~ — **RESOLVED 2026-05-16** (file kept as `run/llama-servers.env` per minimal-scope decision; rename to `config.env` still pending as TODO #2). Drops SCIENTIST_* block, adds `PERSONA_PARALLEL=4`, `UBATCH_SIZE`, `CACHE_TYPE_K/V`. See archive/handoffs/HANDOFF_2026-05-16_2337. | ✅ Done | Migration |
| M4 | ~~Update `start_llama_servers.sh` — single server, `--parallel`, `--cont-batching`, ctx-size scaled by slot count~~ — **RESOLVED 2026-05-16**. Adds `GGML_VK_VISIBLE_DEVICES=0`, `--device Vulkan0`, `--parallel`, `--cont-batching`. Single `start_one` call; scientist removed. See archive/handoffs/HANDOFF_2026-05-16_2337. | ✅ Done | Migration |
| M5 | ~~Update `services/api/server.py` — collapse PERSONA_URL/SCIENTIST_URL to one endpoint, add thinking-mode toggle in payload, remove or raise PERSONA_CONCURRENCY semaphore~~ — **RESOLVED 2026-05-17.** SCIENTIST_URL/PORT removed. `ASYNC_SCIENTIST_*` → `ASYNC_REASONING_*` and `SCIENTIST_INBAND_*` → `REASONING_INBAND_*` (back-compat: old env names still read). `thinking_prefix()` helper added — prepends `/think` or `/no_think` per `THINKING_MODE_DEFAULT` (auto/on/off) and `THINKING_MODE_TOPICS`. `build_persona_prompt()` accepts `thinking_mode` override; param renamed `scientist_notes` → `reasoning_notes`. `PERSONA_CONCURRENCY` default 2→4 (env-driven); `/v1/chat/completions` now also gated by `persona_sem`. `/health` exposes `unified_endpoint`, `async_reasoning_enabled`, `reasoning_inband_*`, `thinking_mode_default`, `thinking_mode_topics` (removes `scientist_endpoint`). `scientist_template()` renamed `reasoning_template()`. **Persona file loader switched to 2-file Hermes naming** in same session: `ensure_profile_files()` scaffolds `SOUL.md` + `.hermes.md` (drops `persona.md`/`style.md`/`system_rules.md`); `load_profile_wrappers()` returns 2-tuple `(soul_md, hermes_md)`; `build_persona_prompt()` prompt prefix uses "Soul (identity, personality, communication style)" and "Hermes rules (hard rules + output format)" sections — Style guide section removed entirely per 2026-05-14 lock. Closes TODO #3 + Phase 1 "Wire SOUL.md + .hermes.md into build_persona_prompt()" incomplete item. See archive/handoffs/HANDOFF_2026-05-17_1030. | ✅ Done | Migration |
| M6 | Parallelize RAG retrieval and worker dispatch with `asyncio.gather` (replace serial in-band reasoning call) | High | Migration |
| M7 | Add `cache_prompt: true` to llama-server request payloads. **Partial:** env wiring deferred to T2.1 sampling overhaul (per-mode sampling presets natural neighbor). | Medium | Migration |
| M8 | ~~Strip `<think>` tags from Qwen3 thinking-mode output before user-facing surface~~ — **superseded by T2.4** (one chokepoint at Task Board → persona surface boundary). Retire when T2.4 lands. | Medium | Migration |
...
| 3 | ~~Wire `SOUL.md` + `.hermes.md` into build_persona_prompt()~~ — **RESOLVED 2026-05-17 (M5 session).** `ensure_profile_files()` now scaffolds the 2-file Hermes-naming profile; `load_profile_wrappers()` returns `(soul_md, hermes_md)`; prompt prefix dropped Style guide section, renamed Persona definition → Soul, System rules → Hermes rules. style.md retired. | ✅ Done | Phase 1 gap |
...
| 37 | **`looks_degenerate()` + self-repair loop — decision gate.** Spec'd historically, never landed in `services/api/server.py` (verified by grep). Decision 2026-05-17: deferred to tier-T2 output-boundary work. Gate: land alongside T2.4 `<think>`-stripping (natural neighbor — both are output-boundary filters), OR formally drop the spec when T2.4 closes. Do NOT close T2.4 with this still open. | Medium | Output safety |
| 38 | **`scripts/status.sh` AI_ROOT + names cleanup** — **RESOLVED 2026-05-17**. AI_ROOT default flipped to `$HOME/Git/Project_Persona`; `names` array trimmed to `("persona")`; legacy names kept in case-exclusion so stale pidfiles do not double-show; scientist references in Config/Models/Endpoints removed. | ✅ Done | Audit |
...
| 2026-05-14 | archive/handoffs/HANDOFF_2026-05-09_0250_single-model-migration.md | M1 Resolution Addendum appended in header. M2a verification result will be referenced via knowledge.md (no separate addendum needed — header is the canonical pointer for migration state). |
...
| 2026-05-15 | knowledge.md | Compatibility Re-Eval Action Items added as new tiered TODO block (T0-T4) at top of TODO section. Sits alongside existing Hermes Adoption (H1-H6) and Single-Model Migration (M1-M12) blocks. Supersedes the analytical H7.x and Q1.x labels from the re-eval (those were enumeration-only). Tiered for incremental validation — finish a tier, verify, move on. T0 is a GO/NO-GO gate (Qwen3.6 arch test + tool-calling verification). T1 is foundation (env_hermes + per-profile config.yaml templating). T2 is core integration (sampling, thinking-mode wiring, preserve_thinking, `<think>` stripping). T3 is operational hardening (doctor.sh, unified_test.sh, ASYNC_REASONING cleanup). T4 is deferred/opt-in (dual-memory, vision, MTP). Reference handoff: archive/handoffs/HANDOFF_2026-05-15_0127_compat-reeval-tiered.md. |
| 2026-05-15 | archive/handoffs/HANDOFF_2026-05-15_0127_compat-reeval-tiered.md | Created — frozen record of the stack compatibility re-evaluation against the Hermes adoption (DECISION 2026-05-11) and the in-flight Qwen3.6 swap consideration. Captures full per-layer findings, the meta-finding (Qwen3.6 honors DECISION 2026-05-09 design intent better than Instruct-2507 did), tiered T0-T4 action plan with acceptance criteria, sequencing rationale, fallback decision branches if Tier 0 fails. |
| 2026-05-15 | HANDOFF.md | Created — **living handoff document**, distinct from dated frozen records. Six sections covering System Status, Critical Path, Active Work & Issues, Open Decisions & Deferred Items, Quick Reference, and Handoff Changelog. Includes Document Maintenance protocol at bottom. Lives at repo root alongside `HANDOFF_YYYY-MM-DD_*.md` frozen records. |
| 2026-05-15 | HANDOFF.md | Restructured for collapsibility + added Section 7 "Retired Ideas & Software". All seven top-level sections wrapped in `<details>`/`<summary>` HTML5 tags; sections 2 (Critical Path) and 3 (Active Work) open by default. Section 7 enumerates explicitly deprecated/deleted/out-of-scope choices grouped by domain with retirement date, reason, replacement, and reference handoff. Three new issues added to Section 3 from consolidation review. |
...
| 2026-05-15 | scripts/regen_handoff_html.sh | Created (chmod +x) — one-command regeneration of HANDOFF.html from HANDOFF.md. Auto-detects pandoc 2.x vs 3.x for `--self-contained` vs `--embed-resources` flag selection. |
| 2026-05-15 | Multiple files | **Consolidation batch (12 actions executed).** Cruft archived to `archive/cruft/`. `AIP_knowledge.md` and `AIP_HANDOFF_mercury_integration_*.md` archived. README.md fully rewritten to reflect locked decisions (Hermes backbone, Qwen3.6 candidate, OpenWebUI primary, Qdrant target, 2-file Hermes-naming profile, single-model topology, AGPLv3+linking exception). `persona/README.md` rewritten for 2-file Hermes-naming profile structure. `.gitignore` rewritten with Hermes-managed paths. knowledge.md `looks_degenerate()` claim corrected (NOT present). Profile content renamed `persona.md` → `SOUL.md`, `system_rules.md` → `.hermes.md`; `style.md` files moved to `archive/legacy_profile_files/`. `README_models_hardware.md` rewritten for single-model topology. |
| 2026-05-16 | run/llama-servers.env, scripts/start_llama_servers.sh (EVO-X2 only) | **Qwen-test first boot.** Rewrote env + launcher on EVO-X2 for the unified Qwen3-30B-A3B-Instruct-2507 Q5_K_M topology: full GPU offload (49/49 layers on Vulkan0), 4 parallel slots × 8192 ctx (32K total), q8_0 KV cache, Flash Attention auto, Hermes 2 Pro chat template, ~63 tok/s gen / ~67 tok/s prompt eval on EVO-X2 iGPU. `GGML_VK_VISIBLE_DEVICES=0` + `--device Vulkan0` pin to RADV/GFX1151. Edits made directly on EVO-X2 (backups: `*.bak.2026-05-16`). Three-state repo reconciliation: stash/pull/pop on Windows with conflict resolution, split into two commits (3910a37 profile rename, 57dad37 docs/handoffs), plus parallel branch `chore/chroma-to-qdrant` at ec2c730. EVO-X2 fast-forwarded to 57dad37, then Qwen3 model moved from `~/Live/AIStack/...` → `~/Git/Project_Persona/models/`. Live workspace `~/Live/AIStack/` quarantined (legacy). See archive/handoffs/HANDOFF_2026-05-16_2337_qwen-test-first-boot.md. |
| 2026-05-17 | run/llama-servers.env, scripts/start_llama_servers.sh, scripts/status.sh, scripts/load_test_m2b.py, KNOWLEDGE.md | **Qwen-test canonicalize + M2b script (0730 PDT session).** Mirrored EVO-X2's 2026-05-16 env + launcher rewrites onto Windows-side `D:\Projects\Git\Project_Persona\` (byte-identical to EVO-X2 versions). Fixed `scripts/status.sh` — AI_ROOT flipped to `~/Git/Project_Persona`, `names` array trimmed to `("persona")`, scientist references removed from Config/Models/Endpoints display. Added `scripts/load_test_m2b.py` (Python + httpx asyncio sustained-load client). KNOWLEDGE.md updated per archive/handoffs/HANDOFF_2026-05-16_2337 §"KNOWLEDGE.md updates needed". M3 + M4 marked ✅ Done; new TODO #37 (looks_degenerate decision gate) and #38 (status.sh cleanup). Decision 2026-05-17: `looks_degenerate()` reinstate deferred to T2-tier work. See archive/handoffs/HANDOFF_2026-05-17_0730_qwen-test-canonicalize.md. |
...
| `run/llama-servers.env` | Runtime config for the unified llama-server (ports, model, ctx, parallel slots, GPU layers, cache types). Rename to `run/config.env` pending (TODO #2). |
...
| `scripts/status.sh` | Quick status summary — unified-topology aware after 2026-05-17 |
| `scripts/start_llama_servers.sh` | Unified llama-server launcher — Vulkan-pinned, 4 parallel slots |
| `scripts/load_test_m2b.py` | M2b sustained-load test (asyncio + httpx, per-minute buckets, JSON report) |
| `scripts/doctor.sh` | Diagnostics + smoke tests — retain, update |
| `scripts/unified_test.sh` | Full end-to-end integration test suite — needs unified-topology rewrite |
| `scripts/init_profiles.sh` | Profile scaffold initializer — 2-file Hermes-naming profile |
